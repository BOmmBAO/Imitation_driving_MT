...
class PredictionModel():
    ...
    def __init__(self, params):
        ...
        self.train_targs = np.array([]).reshape(0,
            self.targ_proc(np.zeros([1, self.dO]), np.zeros([1, self.dO]), self.obs_index).shape[-1]
        )
        ...
        if self.model.is_tf_model:
            ...
            with self.model.sess.graph.as_default():
                self.pre_obs = tf.placeholder(dtype=tf.float32, shape=[29])
                self.cur_obs = tf.placeholder(dtype=tf.float32, shape=[29])
                self.cur_act = tf.placeholder(dtype=tf.float32, shape=[29])
    ...
    def reading_data(self):
        ...
        self.train_num = int(total_num * 0.95)
        ...
        for n in range(self.train_num):
            ...
            new_train_targs = self.targ_proc(obs[:-1], obs[1:], self.obs_index)
        ...
        for n in range(self.test_num):
            ...
            ground_truth = self.targ_proc(obs[:-1], obs[1:], self.obs_index)
    ...
    def cal_test_loss(self):
        ...
        for n in range(len(self.test_obs)):
            pred_obs = self._predict(self.test_obs[n], self.test_acs[n])
            pos_loss, vel_loss = self.loss_function(pred_obs, self.ground_truth[n])
    ...
    def _predict(self, cur_obs, cur_act):

        nopt = tf.shape(cur_act)[0]
        self.sy_cur_obs.load(cur_obs, self.model.sess)
        with self.model.sess.graph.as_default():
            obs_in = tf.tile(self.sy_cur_obs[None], [nopt * self.npart, 1])
            act_in = tf.reshape(tf.tile(cur_act, [nopt * self.npart]), [-1, self.dU])
            next_obs = self._predict_next_obs(obs_in, act_in)
        start_t = time.time()
        pre_obs = self.model.sess.run(next_obs)
        end_t = time.time()
        print('Time cost of a sample:', end_t-start_t, 's')
        return pre_obs
