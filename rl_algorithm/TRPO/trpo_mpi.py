...
def learn(env, policy_func, reward_giver, expert_dataset, rank,
    ...
    if isinstance(env.action_space, spaces.MultiDiscrete):
        ...
        if train_mode == "all":
            ...
            losses = [optimgain, meankl, entbonus, surrgain, meanent, pi_mean, pi_std, steer, throttle_brake, steer_std,
                      throttle_brake_std]
            loss_names = ["optimgain", "meankl", "entloss", "surrgain", "entropy", "pi_mean", "pi_std", "steer",
                          "throttle_brake", "steer_std", "throttle_brake_std"]
    ...
    if expert_dataset is not None:
        ...
        # embedding settings
        OB_VIZ = 'Embedding_ob'
        ob_embedding = tf.get_variable(name=OB_VIZ, shape=[g_step * timesteps_per_batch + expert_dataset.obs.shape[0],
                                                           ob_space.shape[0]])
        ob_ph = tf.placeholder(dtype=tf.float32,
                               shape=[g_step * timesteps_per_batch + expert_dataset.obs.shape[0], ob_space.shape[0]],
                               name="ob_ph")
    ...
    if expert_dataset is not None:
        seg_gen = Runner(env, model, timesteps_per_batch, gamma, lam, length=expert_dataset.num_length,
                         rew_type=rew_type)
    else:
    ...
    while True:
        ...
        if p_update:
            ...
            ep_v, ep_acc, ep_left_offset, ep_right_offset = seg["ep_v"], seg["ep_acc"], seg["ep_left_offset"], seg[
                "ep_right_offset"]
                ...
                if not p_update:
                    ...
                    with timed("searching batch"):
                        if search_mode == 'step':
                            ...
                            ob_expert, ac_expert, search_prop = expert_dataset.search_batch_traj(ob_start, batch_size,
                                                                                                 scene=scene)
