...
def setup(
    ...
    while attempts < num_max_restarts:
        tf.logging.debug("{} out of {} attempts to setup the CARLA simulator".format(
        ...
        tf.logging.debug("Inits a CARLA server at port={}".format(port))
        ...
        # Connect client.
        tf.logging.debug("Connects a CARLA client at port={}".format(port))
        try:
            ...
            tf.logging.debug("Server version: {}".format(client.get_server_version()))
            tf.logging.debug("Client version: {}".format(client.get_client_version()))
            return client, world, frame, server
        except RuntimeError as msg:
            tf.logging.debug(msg)
            attempts += 1
            tf.logging.debug("Stopping CARLA server at port={}".format(port))
