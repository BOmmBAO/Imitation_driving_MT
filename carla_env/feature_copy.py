...
class FeatureExt():
    ...
    def obs_update(self):
        ...
        self.ext_zombiecars_info()
        self.ext_waypoints_info(self.wp_list, self.cur_wp)
    ...
    def ext_egocar_info(self, vehicle):
        ...
        self.info_dict['ego_car_yaw'] = self.vehicle_info.yaw
    ...
    def ext_zombiecars_info(self, total_cars=6):
        ...
        def _get_acc_car(vehicle):
            acc_world = [vehicle.get_acceleration().x, vehicle.get_acceleration().y]
            acc_world = self._rotate_car(acc_world)
            return acc_world

        def get_car_box(vehicle):
            ego_pos = [self.current_loc.x, self.current_loc.y]
            extent_length = common._pos(vehicle.bounding_box.extent)
            box_in_world = self._rotate_zombie2world(vehicle, extent_length)
            box = []
            for p in box_in_world:
                loc = self._transform_car(ego_pos, p)
                box.append(loc)
            return box
        ...
        for car in self.visible_zombie_cars:
            ...
            # vehicles_bounding_box.append(self.get_car_box(car))
            # vehicles_acc.append(_get_acc_car(car))
            if len(vehicles_pos) >= total_cars:
                break
        while len(vehicles_pos) < total_cars:
            vehicles_v.append([0] * 2)
            vehicles_pos.append([0] * 2)
            # vehicles_acc.append([0] * 2)
            # vehicles_bounding_box.append([0] * 8)

        self.info_dict['zombie_cars_pos'] = vehicles_pos
        self.info_dict['zombie_cars_v'] = vehicles_v
        # self.info_dict['zombie_cars_bbox'] = vehicles_pos
        # self.info_dict['zombie_cars_acc'] = vehicles_acc

    def ext_waypoints_info(self, wp_list, cur_wp):
        # Inner lane line
        ...
        self.info_dict['lane_width'] = self.cur_lane_width
        self.info_dict['ego_car_des_vel'] = 20
